...
namespace RoxResources
{
    ...
    public:
        ...
        FILE* access() { RoxMemory::RoxLockGuard lock(m_mutex); return get_lru().access(m_name.c_str()); }

        void free() { RoxMemory::RoxLockGuard lock(m_mutex); get_lru().free(m_name.c_str()); }

        class RoxLru : public RoxMemory::RoxLru<FILE*, 64>
        {
            bool onAccess(const char* name, FILE*& f)
            {
#ifdef _WIN32
                if (!name)
                    return false;
...
namespace RoxResources
{
    ...
    bool RoxFileResourcesProvider::setFolder(const char* name, bool recursive, bool ignore_nonexistent)
    {
        ...
        if (!ignore_nonexistent)
        {
            ...
            {
                RoxLogger::log() << "warning: unable to stat at path " << name << ", probably does not exist\n";
                m_path.push_back('/');
                return false;
            }
            else if (!S_ISDIR(sb.st_mode))
            {
                RoxLogger::log() << "warning: specified path is not a directory " << name << "\n";
                m_path.push_back('/');
                return false;
            }
        }
