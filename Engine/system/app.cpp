...
namespace
{
    ...
    private:
                         ...
                         int y)
        {
            ...
            m_hwnd = CreateWindow("Direct3DWindowClass",
                                  "Direct3D with Fluent UI",
                                  WS_OVERLAPPEDWINDOW,
                                  x,
                                  y,
                                  m_width,
...
namespace {
bool non_native_exit=false;
}

class native_process
{
public:
    static native_process &get() {
        static native_process p; return p;
    }
    
public:
    native_process(): m_started(false) {
    ...
    void start() {
        if(m_started) return; pthread_create(&m_thread,
                                             0,
                                             thread_callback,
                                             0); m_started=true;
    }
...
private:
    static void* thread_callback(void *) {
        main(0,
             0); if(!non_native_exit) exit(0); return 0;
    }
...
struct input_key {
    unsigned int code; bool pressed; unsigned int unicode_char; bool autorepeat;
};
...
{
    JNIEnv *env;
    int r=java_vm->GetEnv((void **)&env,
                          JNI_VERSION_1_6);
    if(r<0 && java_vm->AttachCurrentThread(&env,
                                           NULL)<0)
        return 0;
    return env;
}
...
extern "C"
{
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1spawn_1main(JNIEnv *env,
                                                                     jobject obj)
{
    window=0;
    native_paused=false;
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1pause(JNIEnv *env,
                                                               jobject obj)
{
    native_process::get()
        .lock();
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1resume(JNIEnv *env,
                                                                jobject obj)
{
    native_process::get()
        .lock();
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1touch(JNIEnv *env,
                                                               jobject obj,
                                                               int x,
                                                               int y,
                                                               int id,
...
JNIEXPORT bool JNICALL Java_nya_native_1activity_native_1key(JNIEnv *env,
                                                             jobject obj,
                                                             int code,
                                                             bool pressed,
                                                             int unicode_char,
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1exit(JNIEnv *env,
                                                              jobject obj)
{
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1set_1surface(JNIEnv *env,
                                                                      jobject obj,
                                                                      jobject surface)
{
    if(surface)
    {
        ...
        window=ANativeWindow_fromSurface(env,
                                         surface);
        native_process::get()
            .unlock();
    }
...
JNIEXPORT void JNICALL Java_nya_native_1activity_native_1set_1asset_1mgr(JNIEnv *env,
                                                                         jobject obj,
                                                                         jobject asset_mgr)
{
    nya_resources::apk_resources_provider::set_asset_manager(AAssetManager_fromJava(env,
                                                                                    asset_mgr));
}

JNIEXPORT void JNICALL Java_nya_native_1activity_native_1set_1user_1path(JNIEnv *env,
                                                                         jobject obj,
                                                                         jstring path)
{
...
public:
    bool init(ANativeWindow *window)
    {
        ...
        if(ANativeWindow_setBuffersGeometry(window,
                                            0,
                                            0,
                                            format)!=0)
        {
...
public:
        ...
        while(true)
        {
                ...
                else if(!paused && last_window!=window)
                {
                    ...
                    if(w!=m_renderer.get_width() || h!=m_renderer.get_height())
                    {
                        nya_render::set_viewport(0,
                                                 ...
                                                 .get_height());
                        app
                            .on_resize(m_renderer.get_width(),
                                       m_renderer
                                       .get_height());
                    }
                    last_window=window;
            ...
            for(size_t i=0;i<input_keys.size();++i)
            {
                if(input_keys[i].code>0)
                    app
                    .on_keyboard(input_keys[i].code,
                                 input_keys[i].pressed);
                if(input_keys[i].unicode_char>0)
                    app
                    .on_charcode(input_keys[i].unicode_char,
                                 input_keys[i].pressed,
                                 input_keys[i].autorepeat);
            }
            input_keys
                .clear();
    ...
    void set_virtual_keyboard(int type) 
    {
        ...
        jmethodID setVirtualKeyboard=env->GetStaticMethodID(java_class,
                                                            "setVirtualKeyboard",
                                                            "(I)V");
        env->CallStaticVoidMethod(java_class,
                                  setVirtualKeyboard,
                                  type);
    }
...
extern "C" __attribute__((used)) void emscripten_on_fs_ready() {
    is_fs_ready=true;
}
...
private:
    ...
    static void cursor_pos_callback(GLFWwindow* window,
                                    ...
                                    double ypos)
    {
        get_app().m_app->on_mouse_move(int(xpos),
                                       get_app().m_height-int(ypos));
    }
...
public:
                        ...
                        nya_system::app &app)
    {
        ...
        Colormap cmap=XCreateColormap(m_dpy,
                                      RootWindow(m_dpy,
                                                 vi->screen),
                                      vi->visual,
                                      AllocNone);
                ...
                switch (event.type)
                {
                    ...
                    case ButtonPress:
                    {
                        ...
                        switch (event.xbutton.button)
                        {
                            ...
                            case 6:
                                app
                                    .on_mouse_scroll(scroll_modifier,
                                                     0);
                                break;
